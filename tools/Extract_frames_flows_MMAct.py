...
out_path = r"E:/MMAct_resize_frames/"

def dump_frames(vid_path):
    video = cv2.VideoCapture(vid_path)
    vid_name = vid_path.split('/')[-1].split('.')[0]
    out_full_path = eval(repr(vid_path).replace(r"MMAct_resize", r"MMAct_resize_frames"))[:-4]
    #out_full_path = os.path.join(vid_path[:-4])
...
def main():
    ...
    if Frames_or_Flows=='Frames':
        for i in range(len(vid_list)):
            ...
            if not os.path.isdir(eval(repr(vid_list[i]).replace(r"MMAct_resize", r"MMAct_resize_frames"))[:-4]):
                print("creating folder: "+eval(repr(vid_list[i]).replace(r"MMAct_resize", r"MMAct_resize_frames"))[:-4])
                os.makedirs(eval(repr(vid_list[i]).replace(r"MMAct_resize", r"MMAct_resize_frames"))[:-4])
