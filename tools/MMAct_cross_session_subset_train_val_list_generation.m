...
fid=fopen('MMAct_cross_session_1_8_train_list.txt','w');%写入文件路径
fid2=fopen('MMAct_cross_session_1_8_val_list.txt','w');%写入文件路径
        ...
        for k=1:length(Inertial_data_session_dir)
            ...
            for p=1:length(data_dir)
                ...
                for m=1:length(each_vid_data_dir)
                    ...
                    if ~isempty(strfind(cell2mat(data_dir(p)),'session1')) ||~isempty(strfind(cell2mat(data_dir(p)),'session2')) || ~isempty(strfind(cell2mat(data_dir(p)),'session3'))|| ~isempty(strfind(cell2mat(data_dir(p)),'session4'))
                   ...
                   if ~isempty(strfind(cell2mat(data_dir(p)),'session5')) 
