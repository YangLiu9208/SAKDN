...
opts.load='output/MMAct_Cross_Session_AblationCAM_a_g_o_v_teacher_SemanticFusionVGG16_MMAct_student_TRN_arch_BNInception_seg3_epochs60_batch32_lr0.001_st0.1_se1_dropout0.8/best_acc.pth'
...
def main():
    ...
    for i,(images, labels) in enumerate(video_test_loader, start=1):
        ...
        raw_images=images[:,6:9,:,:].squeeze(0).permute(2,1,0)
        #sub_images = torch.unsqueeze(images[:,12:15,:,:], 0).to(device)
...
def save_image(image,index,label):
    ...
    io.imsave(os.path.join('MMAct_cross_session_visualization', '{}_class{}.jpg'.format(index,label)), image)
